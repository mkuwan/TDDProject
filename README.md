...
開発の手順はいろいろとありますが、クラス、メソッド(宣言だけ)を作成し、メソッドの実装をTDDで行うのが分かりやすいかと思います。
DDDではインターフェースを使用してDIを実現しますが、TDDではメソッドの実装を作成し、それからインターフェースを作るというのもありかもしれません。

アーキテクチャが決まっているなら、クラスやメソッド(実装なし)とインターフェースも作成してから、TDDで実装していくのもありです。

やりやすい方法でトライしていくのがいいと思います。
